...
class CIFAKEDataset(Dataset):
    ...
    def __init__(self, fakeFolder, category=None, transform=None, num_processes=1):
        
        label_1_folders = list(os.listdir(fakeFolder))
        for f in label_1_folders:
            if f[0] == '.':
                label_1_folders.remove(f)
        ...
        xList = [0]*10
        yList = [0]*10
        for i in range(len(label_1_folders)):
            xi, yi = CIFAKEDataset.load_folder(fakeFolder + "/" + label_1_folders[i], 0, category, transform, num_processes)
            xList[i] = xi
            yList[i] = yi
